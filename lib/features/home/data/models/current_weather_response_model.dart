...
extension CurrentWeatherExtension on CurrentWeatherResponseModel {
  CurrentWeatherEntity toEntity() {
    return CurrentWeatherEntity(
      cityName: name ?? "",
      temperature: '${main?.temp ?? 0}',
      weatherDescription:
          (main?.feelsLike ?? 0) >= 15 && (main?.feelsLike ?? 0) < 30
              ? 'Moderate'
              : (main?.feelsLike ?? 0) >= 30
                  ? "Hot"
                  : (main?.feelsLike ?? 0) < 15 && (main?.feelsLike ?? 0) > 5
                      ? 'Cold'
                      : 'Very Cold',
      maxTemperature: '${main?.tempMax ?? 0}',
      minTemperature: '${main?.tempMin ?? 0}',
    );
  }
}
